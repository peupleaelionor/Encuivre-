/**
 * SQL migrations for EN CUIVRE OS (SQLite / better-sqlite3).
 *
 * Migrations are embedded as TS constants (not read from disk) so they work
 * identically in dev, test, CI and the Next server bundle. Each has a stable id;
 * the runner records applied ids in `_migrations` and applies the rest in order,
 * each inside a transaction. A mirror of the SQL lives in
 * `lib/db/migrations/*.sql` for readability.
 *
 * Design note (see CLAUDE.md): scalar and foreign-key columns are normalized;
 * value objects (metrics, memory, incoterms) and short string arrays (materials,
 * grades, documents, riskFlags) are stored as JSON TEXT — a deliberate,
 * documented balance, not "JSON for everything".
 */
